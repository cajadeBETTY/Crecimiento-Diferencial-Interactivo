...
function generarCurvaFromSVG() {
  ...
  elems.forEach(el => {
    if (el.tagName.toLowerCase() === 'path') {
      ...
      let coords = [];
      for (let i = 0; i < list.numberOfItems; i++) coords.push(list.getItem(i));
      for (let i = 0; i < n; i++) {
...
function generarCurvaBase() {
  ...
  if (tipo === 'circulo') for (let i = 0; i < n; i++) {
    const a = TWO_PI * i / n;
...
function fitPoints(pts) {
  ...
  pts.forEach(p => { minX = min(minX, p.x); maxX = max(maxX, p.x);
...
function iniciarCrecimiento() {
  ...
  const n = int(inputPuntos.value());
  const circumference = TWO_PI * float(sliderRadio.value());
  const d = circumference/max(n,1);
...
function reiniciarCrecimiento() {
  running=false; iniciado=false; offsetX=offsetY=0; zoom=1; noiseOffset=0;
