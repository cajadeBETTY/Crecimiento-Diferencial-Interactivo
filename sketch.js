...
function generarCurvaBase() {
  // Generate circle or polygon preview
  points = [];
...
function iniciarCrecimiento() {
  // Start growth from current base
...
function draw() {
  ...
  for (let i=0;i<points.length;i++) {
    ...
    for (let j=0;j<points.length;j++) {
      if(i!==j) {
        let otr=points[j]; let d=dist(act.x,act.y,otr.x,otr.y);
        if(d<minDist) {
    ...
    // Noise
    let type=noiseOffset; // reuse noiseOffset
    let amp=float(sliderAmplitud.value()), freq=float(sliderFrecuencia.value());
    let rnoise=createVector(0,0);
    let tr=tipoRuidoSelect.value();
    if(tr==='perlin') {let n=noise(act.x*freq,act.y*freq+noiseOffset); rnoise=p5.Vector.fromAngle(n*TWO_PI).mult(amp);}
    else if(tr==='perlinImproved') {let nx=noise(act.x*freq,noiseOffset), ny=noise(act.y*freq,noiseOffset+1000); rnoise=createVector((nx-0.5)*amp*2,(ny-0.5)*amp*2);} 
    else if(tr==='valor') rnoise=createVector(random(-1,1)*amp,random(-1,1)*amp);
    else if(tr==='simple') rnoise=p5.Vector.random2D().mult(amp);
    if(cerc>0) {fuer.div(cerc).add(rnoise);} else {fuer=rnoise.copy();}
    act.add(fuer);
    nuevos.push(act);
